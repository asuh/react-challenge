...
export const PokemonCacheProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [cache, setCacheState] = useState<PokemonCache>({ pokemon: {}, abilities: {} });

  const setPokemon = (name: string, details: any) => {
    setCacheState(prev => ({ ...prev, pokemon: { ...prev.pokemon, [name]: details } }));
  };

  const setAbilityEffect = (abilityName: string, effect: string) => {
    setCacheState(prev => ({ ...prev, abilities: { ...prev.abilities, [abilityName]: effect } }));
  };

  const value = useMemo(() => ({ cache, setPokemon, setAbilityEffect }), [cache]);
